function [periods,powers,basis]=mbestgam(s,m,n)

% Calculate the Periodicity Transform using the
% "M-Best" Algorithm (with gamma modification)
%
% syntax:  [periods,powers,basis]=mbestgam(s,m,n)
...
snorms=periodnorm(s)/sqrt(length(s));
...
for i=1:m 
  ...
  for p=1:n
    bas=projectp(s,p);
    if periodnorm(bas)/sqrt(p)>maxnorm
	  maxp=p; maxnorm=periodnorm(bas)/sqrt(p); maxbas=bas;
    end
  end
...
while listchange==1
  ...
  while i<m
	...
	nbigq=periodnorm(xbigq)/sqrt(maxp); nsmallq=periodnorm(xsmallq)/sqrt(maxp);
